...
func (u *Uploader) UploadVideo(videoPath string, metadata VideoMetadata) (string, error) {
...
func GenerateMetadata(input VideoInput, articleTitle string, sourceURL string) VideoMetadata {
    title := articleTitle
    if len(title) > 100 {
        title = title[:97] + "..."
    }
