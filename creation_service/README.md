...
3. Update `.secrets/youtube.env` with `YOUTUBE_CLIENT_ID_<slot>`, `YOUTUBE_CLIENT_SECRET_<slot>`, `YOUTUBE_REFRESH_TOKEN_<slot>`, and set `YOUTUBE_ACCOUNT_SLOT` to the most recently configured slot

To switch accounts at runtime, set `YOUTUBE_ACCOUNT_SLOT` (either edit `.secrets/youtube.env` or export it in your shell before launching the service).

### Environment Variables

```bash
# YouTube OAuth (generated by setup script)
YOUTUBE_ACCOUNT_SLOT=1  # default slot to use when no override is provided
